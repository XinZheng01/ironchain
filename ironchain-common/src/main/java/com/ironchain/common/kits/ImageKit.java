...
public class ImageKit {
     ...
     * @param ouputFileName 缩略图输出路径
     * @param w             缩略图宽
     * @param h             缩略图高
     * @param force         是否强制按照宽高生成缩略图(如果为false，则生成最佳比例缩略图)
     */
    public static void thumbnailImage(File imgFile, String ouputFileName, int w, int h, boolean force){
    	if(imgFile.exists()){
            try {
                ...
                g.dispose();
                // 将图片保存在原目录并加上前缀
                ImageIO.write(bi, suffix, new File(ouputFileName));
            } catch (IOException e) {
    ...
    public static void thumbnailImage(File imgFile, int w, int h, String prevfix, boolean force){
    	...
    	thumbnailImage(imgFile, p.substring(0,p.lastIndexOf(File.separator)) + File.separator + prevfix + imgFile.getName(),
    			w, h, force);
    }
    ...
    public static void thumbnailImage(File imgFile, String ouputFileName, int w, int h){
        thumbnailImage(imgFile, ouputFileName, w, h, false);
    }
