...
<body>
	<div>
		...
		<div class="page">
			<div class="panel">
				<form id="searchForm" action="${ctx}/letter/list" method="get" class="search-form form-inline">
				<div class="panel-body">
					  <div class="form-group">
					    <input type="text" name="srch_LIKE_title" class="form-control" value="${srch_LIKE_title}" placeholder="标题">
					  </div>
					  <div class="form-group">
			 	...
			 	<sec:authorize url="/letter/add">
			 		<button class="btn btn-primary" type="button" onclick="javascript:location.href='${ctx}/letter/add';"><i class="icon icon-plus"></i>新增</button>
			 	</sec:authorize>
			 	</div>
	                ...
	                <tbody>
						<c:forEach items="${page.content}" var="item">
							<tr>
								...
								<td>${item.number}</td>
								<td>${item.typeStr}</td>
								<td>${item.sendTypeStr}</td>
								<td><fmt:formatDate value="${item.createTime}" pattern="yyyy-MM-dd HH:mm"/></td>
								<td>
								<sec:authorize url="/letter/edit">
									<a href="${ctx}/letter/edit?id=${item.id}">查看</a>
								</sec:authorize>
								</td>
							</tr>
						</c:forEach>
